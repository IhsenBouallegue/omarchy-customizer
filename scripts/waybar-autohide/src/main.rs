...
const PIXEL_THRESHOLD: i32 = 5; // Sensitivity
const PIXEL_THRESHOLD_SECONDARY: i32 = 50;
const MOUSE_REFRESH_DELAY_MS: u64 = 100;
const LOCK_FILE: &str = "/tmp/waybar-disabled";
...
fn main() {
    ...
    for event in rx {
        // 1. CHECK MANUAL OVERRIDE (Lock File)
        if std::path::Path::new(LOCK_FILE).exists() {
             if last_visibility {
                if waybar_pid.is_none() { waybar_pid = find_waybar_pid(); }
                if let Some(pid) = waybar_pid {
                    set_waybar_visible(pid, false); // Force Hide
                }
                last_visibility = false;
             }
             continue; // Skip logic while locked
        }
        ...
        loop {
            if let (Some(pos), Some(monitors)) = (get_cursor_pos(), get_monitors()) {
                let active_monitor = monitors.iter().find(|m| {
                    pos.x >= m.x && pos.x <= m.x + m.width && pos.y >= m.y && pos.y <= m.y + m.height
                });
                ...
                if let Some(m) = active_monitor {
                    ...
                    let threshold = if previous_state { PIXEL_THRESHOLD_SECONDARY } else { PIXEL_THRESHOLD };
...
#[derive(Debug)]
enum Event { CursorTop(bool), WindowsOpened(bool) }

fn hypr_query(cmd: &str) -> Option<String> {
    let socket_path = format!("{}/hypr/{}/.socket.sock",
...
fn get_cursor_pos() -> Option<CursorPos> { serde_json::from_str(&hypr_query("j/cursorpos")?).ok() }
fn get_monitors() -> Option<Vec<Monitor>> { serde_json::from_str(&hypr_query("j/monitors")?).ok() }
...
fn find_waybar_pid() -> Option<i32> {
    fs::read_dir("/proc").ok()?.filter_map(|entry| {
...
#[derive(Deserialize)]
struct Monitor { x: i32, y: i32, width: i32, height: i32 }

#[derive(Parser, Clone)]
struct Args {
    #[arg(long, default_value_t = false)] always_hidden: bool,
    #[arg(long, value_enum, default_value = "top")] side: Side,
}

#[derive(Debug, Clone, Copy, Deserialize, ValueEnum)]
enum Side { Top, Left, Right, Bottom }
impl Default for Side { fn default() -> Self { Side::Top } }
